...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['login'])) {
        ...
        if (!empty($email) && !empty($pass)) {
            [$stmt, $res, $conn] = q("SELECT id, name, email, password, role FROM users WHERE email = ?", 's', [$email]);
        ...
        if (!empty($name) && !empty($email) && !empty($pass) && !empty($confirm_pass)) {
            if ($pass !== $confirm_pass) {
                ...
                [$check_stmt, $check_res, $conn] = q("SELECT id FROM users WHERE email = ?", 's', [$email]);
                ...
                if ($check_res->num_rows > 0) {
                    ...
                    [$insert_stmt, $insert_res, $conn] = q("INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)", 'ssss', [$name, $email, $hashed_password, $role]);
                    
                    if ($insert_stmt) {
                        $message = 'Registration successful! You can now login.';
                        echo "<script>setTimeout(() => showLogin(), 1000);</script>";
                    } else {
